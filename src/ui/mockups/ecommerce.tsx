...
export const EcommerceMock = () => {
  ...
  return (
    <div className="space-y-8">
      <header className="flex justify-between items-center py-4 px-8 bg-[var(--color-surface)] border-b border-[var(--color-role-neutral-border)] sticky top-0 z-10">
        <div className="text-2xl font-black italic tracking-tighter text-[var(--color-text-primary)]">GALAKTYCZNY TOST</div>
        <div className="flex gap-8 items-center font-bold text-sm text-[var(--color-text-muted)]">
        ...
        <Grid cols={3}>
          {[
            ...
            <Card key={prod.name} id={prod.id}>
              <div className="aspect-square bg-[var(--color-surface-raised)] rounded-[var(--radius-base)] mb-4 flex items-center justify-center text-4xl border border-[var(--color-role-neutral-border)]">
                🍞
              </div>
              ...
              <div className="text-2xl font-black text-[var(--color-text-primary)] mb-6">{prod.price}</div>
      ...
      <footer className="bg-[var(--color-surface-raised)] p-12 border-t border-[var(--color-role-neutral-border)] mt-24">
          <Grid cols={4}>
              ...
              <div className="space-y-2 text-sm font-bold text-[var(--color-text-secondary)]">
                  <div className="text-xs font-black uppercase text-[var(--color-text-muted)] mb-2">Sklep</div>
                  <div>Menu</div>
                  <div>Dodatki</div>
                  <div>Subskrypcja</div>
              </div>
              <div className="space-y-2 text-sm font-bold text-[var(--color-text-secondary)]">
                  <div className="text-xs font-black uppercase text-[var(--color-text-muted)] mb-2">Wsparcie</div>
                  <div>Dostawa</div>
                  <div>Reklamacje</div>
                  <div>FAQ</div>
              </div>
