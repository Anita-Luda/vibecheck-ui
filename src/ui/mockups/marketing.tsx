...
export const MarketingMock = () => (
    <div className="bg-[var(--color-bg)]">
        ...
        <nav className="flex items-center justify-between px-12 py-6 bg-[var(--color-surface)]/80 backdrop-blur-md sticky top-0 z-50 border-b border-[var(--color-role-neutral-border)]">
            ...
            <div className="flex gap-12 font-bold text-sm text-[var(--color-text-muted)]">
        ...
        {/* Hero Section */}
        <section className="px-12 py-32 text-center max-w-5xl mx-auto space-y-8">
            ...
            <div className="pt-24 space-y-4">
                ...
                <div className="flex justify-center gap-16 opacity-30 grayscale items-center">
        ...
        <section className="px-12 py-24 bg-[var(--color-surface-raised)]">
            <h2 className="text-4xl font-black text-center mb-16 tracking-tight">System Silniejszy niż Grawitacja</h2>
            <div className="grid grid-cols-4 gap-6 h-[600px]">
                <Card id={0} className="col-span-2 row-span-2 flex flex-col justify-end p-8 bg-[var(--color-role-primary)] text-white">
                    <h3 className="text-4xl font-black mb-4 leading-none">OKLCH: Czysta Percepcja</h3>
                ...
                <Card id={1} className="row-span-1 bg-[var(--color-surface)]">
                    ...
                    <p className="text-xs text-[var(--color-text-muted)] mt-2">Ochrona przed spadkami nastroju w interfejsie.</p>
                </Card>
                <Card id={2} className="row-span-1 bg-[var(--color-surface)]">
                    <div className="text-4xl mb-4">🥔</div>
                    <h4 className="font-black text-lg">PotatOS Engine</h4>
                    <p className="text-xs text-[var(--color-text-muted)] mt-2">Optymalizacja pod kątem bulw wysokiej gęstości.</p>
                </Card>
                <Card id={3} className="col-span-2 row-span-1 bg-[var(--color-role-accent)] text-white flex items-center gap-8">
            ...
            <Grid cols={3}>
                {[
                    ...
                    <div key={p.name} className={`p-12 rounded-[var(--radius-lg)] border-2 transition-all ${p.popular ? 'border-[var(--color-role-accent)] scale-105 shadow-2xl z-10' : 'border-[var(--color-role-neutral-border)]'} bg-[var(--color-surface)] flex flex-col`}>
                        ...
                        <p className="text-xs text-[var(--color-text-muted)] mb-8 font-bold">{p.desc}</p>
                        <div className="flex items-end gap-1 mb-8">
                            <span className="text-5xl font-black">${p.price}</span>
                            <span className="text-sm text-[var(--color-text-muted)] font-bold mb-2">/miesięcznie</span>
                        </div>
                        <ul className="flex-1 space-y-4 mb-12">
                            {p.features.map(f => <li key={f} className="text-sm font-bold flex gap-2"><span>✓</span> {f}</li>)}
                        </ul>
        ...
        {/* FAQ Accordion */}
        <section className="max-w-3xl mx-auto px-12 py-24 space-y-12">
            <h2 className="text-4xl font-black text-center mb-12">Często Zadawane Pytania (FAQ)</h2>
        ...
        <footer className="bg-[var(--color-surface)] border-t border-[var(--color-role-neutral-border)] py-24 px-12">
            <Grid cols={4}>
                ...
                <div>
                    <h5 className="text-[10px] font-black uppercase text-[var(--color-text-muted)] tracking-widest mb-6">Produkt</h5>
                ...
                <div>
                    <h5 className="text-[10px] font-black uppercase text-[var(--color-text-muted)] tracking-widest mb-6">Firma</h5>
                    <div className="space-y-4 text-sm font-bold">
                        <div>O nas</div>
                        <div>Kariera</div>
                        <div>Kontakt</div>
                ...
                <div className="space-y-6">
                    <h5 className="text-[10px] font-black uppercase text-[var(--color-text-muted)] tracking-widest mb-6">Zapisz się do wajbu</h5>
            ...
            <div className="mt-24 pt-8 border-t border-[var(--color-role-neutral-border)] flex justify-between items-center text-[10px] font-black uppercase text-[var(--color-text-muted)]">
