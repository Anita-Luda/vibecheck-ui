...
export const AIAppMock = () => {
    ...
    return (
        <div className="flex h-[800px] border border-[var(--color-role-neutral-border)] rounded-[var(--radius-lg)] overflow-hidden bg-[var(--color-bg)]">
            {/* Sidebar */}
            <aside className="w-64 border-r border-[var(--color-role-neutral-border)] bg-[var(--color-surface-raised)] p-4 flex flex-col gap-6">
                ...
                <div className="pt-4 border-t border-[var(--color-role-neutral-border)] flex items-center gap-2">
            ...
            <main className="flex-1 flex flex-col relative bg-[var(--color-surface)]">
                <header className="p-4 border-b border-[var(--color-role-neutral-border)] flex justify-between items-center bg-[var(--color-surface-raised)]/50 backdrop-blur">
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-6">
                    {messages.map((m, i) => (
                        <div key={i} className={`flex gap-4 ${m.role === 'user' ? 'justify-end' : ''}`}>
                            ...
                            <div className={`max-w-[70%] p-4 rounded-[var(--radius-base)] text-sm shadow-sm ${
                                m.role === 'user'
                                ...
                                <div className="font-black text-[10px] uppercase mb-1 opacity-50">{m.role}</div>
                                {m.text}
                                {m.role === 'assistant' && i === messages.length - 1 && (
                                    <div className="mt-4 pt-4 border-t border-[var(--color-role-neutral-border)] flex gap-2">
                    ...
                    <div className="flex gap-4">
                        ...
                        <div className="bg-[var(--color-surface-raised)] p-4 rounded-[var(--radius-base)] border border-[var(--color-role-neutral-border)]">
                            <div className="flex items-center gap-2 text-xs font-bold italic text-[var(--color-text-muted)]">
                ...
                <div className="p-8 bg-gradient-to-t from-[var(--color-surface)] to-transparent">
                    <div className="max-w-3xl mx-auto relative">
                        <textarea
                            className="w-full bg-[var(--color-surface-raised)] border-2 border-[var(--color-role-neutral-border)] rounded-[var(--radius-lg)] p-4 pr-16 text-sm outline-none focus:border-[var(--color-role-accent)] transition-all resize-none shadow-xl"
                            placeholder="Zadaj pytanie o wajb..."
                            rows={3}
                        />
                        <button className="absolute right-4 bottom-4 w-10 h-10 rounded-full bg-[var(--color-role-accent)] text-white font-bold shadow-lg hover:scale-110 transition-transform">
                            ↑
                        </button>
                    ...
                    <p className="text-center text-[10px] text-[var(--color-text-muted)] mt-4 font-bold uppercase tracking-widest">
